...
public class Main {
    ...
    private static double[] applyNetwork(double[] data) {
        ...
        double[] resCouche = new double[nbNeurones[1]];
        for (int j = 0; j < nbNeurones[1]; j++) {
            for (int k = 0; k < nbNeurones[0]; k++) {
                resCouche[j] += poids[0][j][k] * (k == 0 ? 1 : data[k - 1]);
            }
        }

        data = resCouche;

        for (int coucheIndex = 1; coucheIndex < nbCouches; coucheIndex++) {
            resCouche = new double[nbNeurones[coucheIndex]];
            for (int currentNeuroneIndex = 0; currentNeuroneIndex < nbNeurones[coucheIndex]; currentNeuroneIndex++) {
                for (int neuronePrecedentIndex = 0; neuronePrecedentIndex < nbNeurones[coucheIndex - 1]; neuronePrecedentIndex++) {
                    resCouche[currentNeuroneIndex] += poids[coucheIndex - 1][currentNeuroneIndex][neuronePrecedentIndex] * data[neuronePrecedentIndex];
                }
            }
            data = resCouche;
        }
